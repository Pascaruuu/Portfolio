...
`vite.config.ts`: `sveltekit()` + the custom `generatedBreakpoints()` plugin + `postcssCustomMedia()`. `svelte.config.js`: `adapter-auto`, runes forced on, plus a `[vitePreprocess(), mdsvex()]` preprocess chain scoped to the `.svx` extension only — compiles per-project detail write-ups (`src/lib/content/projects/*/detail.{en,ja}.svx`) into lazily-loaded Svelte components (see `src/lib/content/projects/loader.ts`, `types.ts`'s `DetailLoader`, and `ProjectDetail.svelte`'s load-state handling). PHASE 5E added a second lazy `import.meta.glob` alongside the detail-file one, for body images referenced from inside those write-ups (`loader.ts`'s `bodyImageModules`, `types.ts`'s `BodyImageLoader`) — same non-eager reasoning: an image embedded in a write-up shouldn't ship in the bundle any more eagerly than the write-up itself. Resolved per-project into a `bodyImages: Record<filename, BodyImageLoader>` passed to the compiled detail component as an `images` prop; a filename lookup miss (typo, or the project's own card-image filename, which is deliberately excluded) is `undefined`, not an error — `BodyImage.svelte` renders nothing for it. `package.json` scripts: `dev`, `build`, `preview`, `prepare`, `check` (svelte-kit sync + svelte-check), `check:watch`, `lint`.
