...
`vite.config.ts`: `sveltekit()` + the custom `generatedBreakpoints()` plugin + `postcssCustomMedia()`. `svelte.config.js`: `adapter-auto`, runes forced on, plus a `[vitePreprocess(), mdsvex()]` preprocess chain scoped to the `.svx` extension only — compiles per-project detail write-ups (`src/lib/content/projects/*/detail.{en,ja}.svx`) into lazily-loaded Svelte components (see `src/lib/content/projects/loader.ts`, `types.ts`'s `DetailLoader`, and `ProjectDetail.svelte`'s load-state handling). PHASE 5E added a second lazy `import.meta.glob` for images referenced from inside those write-ups (`loader.ts`'s `bodyImageModules`, `types.ts`'s `BodyImageLoader`) — same non-eager reasoning: an image embedded in a write-up shouldn't ship in the bundle any more eagerly than the write-up itself. PHASE 5G repurposed that glob (bumping its width to 1844px) to ALSO serve `ProjectItem.detailImage`, the detail view's header image, looked up by the card image's own filename instead of excluded by it. PHASE 7 un-shared it again: `bodyImageModules` (1844px, unchanged) now feeds `detailImage` exclusively; a new third glob, `writeupBodyImageModules` (850px — re-derived for the 700px-capped float body images now use, see `app.css`'s `.body-image` and that glob's own comment for the arithmetic), feeds the `bodyImages: Record<filename, BodyImageLoader>` record passed to the compiled detail component as an `images` prop. A filename lookup miss (typo, or the project's own card-image filename, which is deliberately excluded from `bodyImages`) is `undefined`, not an error — `BodyImage.svelte` renders nothing for it. The card grid's own `imageModules` glob (402px, eager) is unchanged throughout all of this — card, header image, and write-up body image are three separate derivatives of whatever source files a project directory holds, not one shared `Picture`. PHASE 6 added an optional per-project `focalPoint: { x, y }` to `meta.json` (validated in `validate.ts`, degrading to a centered crop — `null` — when absent or malformed, the one field in this schema that warns only on malformed input, never on absence) — passed through `ProjectItem.focalPoint` into `ProjectDetail.svelte` as CSS custom properties consumed by `.project-detail-img`'s `object-position`, steering where the header strip's cover-crop anchors. `package.json` scripts: `dev`, `build`, `preview`, `prepare`, `check` (svelte-kit sync + svelte-check), `check:watch`, `lint`.
